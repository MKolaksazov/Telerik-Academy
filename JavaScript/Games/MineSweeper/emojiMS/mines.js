...
function reveal(index)        // Uncover the tile
        ...
        if(board[index]!='mine'&&picture(index)=="x")        // If it's covered and not a mine:
                ...
                {
                if(x>0&&picture(index-1) == "⬛")        reveal(index-1);                                        // left

                if(x<(columns-1)&&picture(+index+1) == "⬛") reveal(+index+1);                                // right

                if(y<(rows-1)&&picture(+index+columns) == "⬛") reveal(+index+columns);                        // down

                if(y>0&&picture(index-columns) == "⬛") reveal(index-columns);                                // up

                if(x>0&&y>0&&picture(index-columns-1) == "⬛") reveal(index-columns-1);                        // up & left

                if(x<(columns-1)&&y<(rows-1)&&picture(+index+columns+1) == "⬛") reveal(+index+columns+1);        // down & right

                if(x>0&&y<(rows-1)&&y<(rows-1)&&picture(+index+columns-1) == "⬛") reveal(+index+columns-1);                // down & left

                if(x<(columns-1)&&y>0&&y<(rows-1)&&picture(+index-columns+1) == "⬛") reveal(+index-columns+1);                // up & right
